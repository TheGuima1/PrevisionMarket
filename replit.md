...
### UI/UX Decisions
The platform follows a **Paleta C - Futurista/Crypto** design with deep purple background (#130F1E), neon purple accents (#7A4CFF), and neon action colors (blue #386BFF for YES, red #FF3F55 for NO). The design prioritizes tech innovation, high contrast, and crypto-friendly aesthetics through clear text (#E8E8FF), purple neutral cards (#1A1922), and neon highlights. Typography includes Inter for body text, Manrope for headings, and Roboto Mono for numerical values. Light mode features a deep purple background (#130F1E) with purple neutral cards (#1A1922); dark mode uses an even darker background (252 40% 5%) with brighter neon colors. YES/NO buttons feature vibrant neon gradients with colored shadows. All components strictly follow `design_guidelines.md`, ensuring futuristic consistency, Shadcn UI component usage, neon elevation system, full responsiveness, and visual states for loading, error, and empty content.
...
- **Replit Autoscale Health Checks**: GET `/health` endpoint provides health check with database connectivity validation in JSON format. Frontend served via standard Express static/Vite middleware without route conflicts.
