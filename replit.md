...
- **Replit Autoscale Health Checks**: Dedicated GET `/` endpoint responds in 11ms with "200 OK" status for Replit Autoscale health checks. Route is registered before all static routes to ensure priority handling. Separate `/health` endpoint provides detailed DB status in JSON format.
