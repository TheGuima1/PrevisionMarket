...
### UI/UX Decisions
The platform follows a **Purple Tech Premium** design with clean white backgrounds (#FAFAFF), vibrant purple tech accents (#7A4CFF), and action colors (purple #7A4CFF for YES, red #EF4444 for NO). The design prioritizes modern fintech aesthetics, clarity, and professionalism through subtle purple gradients, clean white cards, and soft purple shadows. Typography includes Inter for body text, Manrope for headings, and Roboto Mono for numerical values. Light mode features white backgrounds with subtle purple gradients (#FAFAFF to #F3EDFF); dark mode uses deep purple backgrounds (260 40% 5%) with brighter neon purple colors. YES/NO buttons feature clean solid colors with hover elevations. All components strictly follow `design_guidelines.md`, ensuring premium fintech consistency, Shadcn UI component usage, elevation system, full responsiveness, and visual states for loading, error, and empty content.
